    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback))
{
  ...
  for (i = 0;; i++)
  {
    ...
    if (opRes == NExtract::NOperationResult::kOK)
    {
      ...
      RINOK(CreateItemInStream(index, pos, size, &inStream))
...
HRESULT CHandlerCont::CreateItemInStream(UInt32 /* index */, UInt64 pos, UInt64 size, ISequentialInStream **stream)
{
...
Z7_COM7F_IMF(CHandlerCont::GetStream(UInt32 index, ISequentialInStream **stream))
{
  ...
  *stream = NULL;
  ...
  if (GetItem_ExtractInfo(index, pos, size) != NExtract::NOperationResult::kOK)
    return S_FALSE;
  return CreateItemInStream(index, pos, size, stream);
  COM_TRY_END
}
